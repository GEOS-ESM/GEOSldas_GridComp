...
contains
  ...
  function new_sphere_random_fields(pert_param, pert_grid_f, comm, rc, c2) result(rf)
    ...
    write(id_string,'(i0,":",i0,":",es16.8)') rf%nz_sphere, rf%nphi_sphere, xcorr_deg
  ...
  subroutine sphere_to_latlon_field(rfield_sphere, sf, N_lon, N_lat, rfield_latlon)
    ...
    real(c_double) :: v1, v2
    real(c_double), parameter :: PI = 3.141592653589793d0
    real(c_double), parameter :: D2R = PI / 180.0d0
    do j_lat = 1, N_lat
  ...
  subroutine xcorr_deg_to_c2(xcorr_deg, nz, c2_out)
    ...
    do iter = 1, max_iter
      ...
      if (abs(theta_mid - xcorr_rad) < tol_deg * D2R) exit
  ...
  contains
    ...
    real(c_double) function compute_efold_theta(c2_val, lmax_in)
      ...
      real(c_double), parameter :: inv4pi = 0.25d0 / 3.141592653589793d0
      ...
      compute_efold_theta = 3.141592653589793d0
      do iz = 0, nz_scan
