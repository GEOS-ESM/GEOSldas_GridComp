...
contains
  ...
  subroutine RUN (gc, import, export, clock, rc)
    ...
    if (associated(ICESMB_enavg)      .and. associated(ICESMB))     ICESMB_enavg   = ICESMB_enavg    + ICESMB
    if (associated(ICESURF_enavg)     .and. associated(ICESURF))    ICESURF_enavg  = ICESURF_enavg   + ICESURF
    if (associated(ICETHICK_enavg)    .and. associated(ICETHICK))   ICETHICK_enavg = ICETHICK_enavg  + ICETHICK
    if (associated(ICEVEL_enavg)      .and. associated(ICEVEL))     ICEVEL_enavg   = ICEVEL_enavg    + ICEVEL
    if (associated(EMIS_enavg)        .and. associated(EMIS))          EMIS_enavg  = EMIS_enavg      + EMIS
    if (associated(ALBVR_enavg)       .and. associated(ALBVR))         ALBVR_enavg = ALBVR_enavg     + ALBVR
    if (associated(ALBVF_enavg)       .and. associated(ALBVF))         ALBVF_enavg = ALBVF_enavg     + ALBVF
    if (associated(ALBNR_enavg)       .and. associated(ALBNR))         ALBNR_enavg = ALBNR_enavg     + ALBNR
    if (associated(ALBNF_enavg)       .and. associated(ALBNF))         ALBNF_enavg = ALBNF_enavg     + ALBNF
    if (associated(TST_enavg)         .and. associated(TST))             TST_enavg = TST_enavg       + TST
    if (associated(LST_enavg)         .and. associated(LST))             LST_enavg = LST_enavg       + LST
    if (associated(QST_enavg)         .and. associated(QST))             QST_enavg = QST_enavg       + QST
    if (associated(TH_enavg)          .and. associated(TH))               TH_enavg = TH_enavg        + TH
    if (associated(QH_enavg)          .and. associated(QH))               QH_enavg = QH_enavg        + QH
    if (associated(DELTS_enavg)       .and. associated(DELTS))         DELTS_enavg = DELTS_enavg     + DELTS
    if (associated(DELQS_enavg)       .and. associated(DELQS))         DELQS_enavg = DELQS_enavg     + DELQS
    if (associated(CHT_enavg)         .and. associated(CHT))             CHT_enavg = CHT_enavg       + CHT
    if (associated(CMT_enavg)         .and. associated(CMT))             CMT_enavg = CMT_enavg       + CMT
    if (associated(CQT_enavg)         .and. associated(CQT))             CQT_enavg = CQT_enavg       + CQT
    if (associated(CNT_enavg)         .and. associated(CNT))             CNT_enavg = CNT_enavg       + CNT
    if (associated(RIT_enavg)         .and. associated(RIT))             RIT_enavg = RIT_enavg       + RIT
    if (associated(ACCUM_enavg)       .and. associated(ACCUM))         ACCUM_enavg = ACCUM_enavg     + ACCUM
    if (associated(EVPICE_GL_enavg)   .and. associated(EVPICE_GL)) EVPICE_GL_enavg = EVPICE_GL_enavg + EVPICE_GL
    if (associated(SUBLIM_enavg)      .and. associated(SUBLIM))       SUBLIM_enavg = SUBLIM_enavg    + SUBLIM
    if (associated(SNOMAS_GL_enavg)   .and. associated(SNOMAS_GL)) SNOMAS_GL_enavg = SNOMAS_GL_enavg + SNOMAS_GL
    if (associated(SNOWMASS_enavg)    .and. associated(SNOWMASS))   SNOWMASS_enavg = SNOWMASS_enavg  + SNOWMASS
    if (associated(SNOWDP_GL_enavg)   .and. associated(SNOWDP_GL)) SNOWDP_GL_enavg = SNOWDP_GL_enavg + SNOWDP_GL
    if (associated(ASNOW_GL_enavg)    .and. associated(ASNOW_GL))   ASNOW_GL_enavg = ASNOW_GL_enavg  + ASNOW_GL
    if (associated(WESNEXT_enavg)     .and. associated(WESNEXT))     WESNEXT_enavg = WESNEXT_enavg   + WESNEXT
    if (associated(WESNSC_enavg)      .and. associated(WESNSC))       WESNSC_enavg = WESNSC_enavg    + WESNSC
    if (associated(SNDZSC_enavg)      .and. associated(SNDZSC))       SNDZSC_enavg = SNDZSC_enavg    + SNDZSC
    if (associated(WESNPREC_enavg)    .and. associated(WESNPREC))   WESNPREC_enavg = WESNPREC_enavg  + WESNPREC
    if (associated(SNDZPREC_enavg)    .and. associated(SNDZPREC))   SNDZPREC_enavg = SNDZPREC_enavg  + SNDZPREC
    if (associated(SNDZ1PERC_enavg)   .and. associated(SNDZ1PERC)) SNDZ1PERC_enavg = SNDZ1PERC_enavg + SNDZ1PERC
    if (associated(WESNBOT_enavg)     .and. associated(WESNBOT))     WESNBOT_enavg = WESNBOT_enavg   + WESNBOT
    if (associated(RAINRFZ_enavg)     .and. associated(RAINRFZ))     RAINRFZ_enavg = RAINRFZ_enavg   + RAINRFZ
    if (associated(SMELT_enavg)       .and. associated(SMELT))         SMELT_enavg = SMELT_enavg     + SMELT
    if (associated(IMELT_enavg)       .and. associated(IMELT))         IMELT_enavg = IMELT_enavg     + IMELT
    if (associated(SNOWALB_enavg)     .and. associated(SNOWALB))     SNOWALB_enavg = SNOWALB_enavg   + SNOWALB
    if (associated(SNICEALB_enavg)    .and. associated(SNICEALB))   SNICEALB_enavg = SNICEALB_enavg  + SNICEALB
    if (associated(MELTWTR_enavg)     .and. associated(MELTWTR))     MELTWTR_enavg = MELTWTR_enavg   + MELTWTR
    if (associated(MELTWTRCONT_enavg) .and. associated(MELTWTRCONT)) MELTWTRCONT_enavg = MELTWTRCONT_enavg + MELTWTRCONT
    if (associated(LWC_enavg)         .and. associated(LWC))             LWC_enavg = LWC_enavg       + LWC
    if (associated(RUNOFF_enavg)      .and. associated(RUNOFF))       RUNOFF_enavg = RUNOFF_enavg    + RUNOFF
    if (associated(GUST_enavg)        .and. associated(GUST))           GUST_enavg = GUST_enavg      + GUST
    if (associated(VENT_enavg)        .and. associated(VENT))           VENT_enavg = VENT_enavg      + VENT
    if (associated(Z0_enavg)          .and. associated(Z0))               Z0_enavg = Z0_enavg        + Z0
    if (associated(Z0H_enavg)         .and. associated(Z0H))             Z0H_enavg = Z0H_enavg       + Z0H
    if (associated(MOT2M_enavg)       .and. associated(MOT2M))         MOT2M_enavg = MOT2M_enavg     + MOT2M
    if (associated(MOQ2M_enavg)       .and. associated(MOQ2M))         MOQ2M_enavg = MOQ2M_enavg     + MOQ2M
    if (associated(MOU2M_enavg)       .and. associated(MOU2M))         MOU2M_enavg = MOU2M_enavg     + MOU2M
    if (associated(MOV2M_enavg)       .and. associated(MOV2M))         MOV2M_enavg = MOV2M_enavg     + MOV2M
    if (associated(MOT10M_enavg)      .and. associated(MOT10M))       MOT10M_enavg = MOT10M_enavg    + MOT10M
    if (associated(MOQ10M_enavg)      .and. associated(MOQ10M))       MOQ10M_enavg = MOQ10M_enavg    + MOQ10M
    if (associated(MOU10M_enavg)      .and. associated(MOU10M))       MOU10M_enavg = MOU10M_enavg    + MOU10M
    if (associated(MOV10M_enavg)      .and. associated(MOV10M))       MOV10M_enavg = MOV10M_enavg    + MOV10M
    if (associated(MOU50M_enavg)      .and. associated(MOU50M))       MOU50M_enavg = MOU50M_enavg    + MOU50M
    if (associated(MOV50M_enavg)      .and. associated(MOV50M))       MOV50M_enavg = MOV50M_enavg    + MOV50M
    ...
    if (associated(HLWUP_enavg)       .and. associated(HLWUP))         HLWUP_enavg = HLWUP_enavg     + HLWUP
    ...
    if (associated(DNICFLX_enavg)     .and. associated(DNICFLX))     DNICFLX_enavg = DNICFLX_enavg   + DNICFLX
    if (associated(GHSNOW_enavg)      .and. associated(GHSNOW))       GHSNOW_enavg = GHSNOW_enavg    + GHSNOW
    if (associated(GHTSKIN_enavg)     .and. associated(GHTSKIN))     GHTSKIN_enavg = GHTSKIN_enavg   + GHTSKIN
    if (associated(ITY_enavg)         .and. associated(ITY))             ITY_enavg = ITY_enavg       + ITY
    if (associated(RMELTDU001_enavg)  .and. associated(RMELTDU001)) RMELTDU001_enavg = RMELTDU001_enavg + RMELTDU001
    if (associated(RMELTDU002_enavg)  .and. associated(RMELTDU002)) RMELTDU002_enavg = RMELTDU002_enavg + RMELTDU002
    if (associated(RMELTDU003_enavg)  .and. associated(RMELTDU003)) RMELTDU003_enavg = RMELTDU003_enavg + RMELTDU003
    if (associated(RMELTDU004_enavg)  .and. associated(RMELTDU004)) RMELTDU004_enavg = RMELTDU004_enavg + RMELTDU004
    if (associated(RMELTDU005_enavg)  .and. associated(RMELTDU005)) RMELTDU005_enavg = RMELTDU005_enavg + RMELTDU005
    if (associated(RMELTBC001_enavg)  .and. associated(RMELTBC001)) RMELTBC001_enavg = RMELTBC001_enavg + RMELTBC001
    if (associated(RMELTBC002_enavg)  .and. associated(RMELTBC002)) RMELTBC002_enavg = RMELTBC002_enavg + RMELTBC002
    if (associated(RMELTOC001_enavg)  .and. associated(RMELTOC001)) RMELTOC001_enavg = RMELTOC001_enavg + RMELTOC001
    if (associated(RMELTOC002_enavg)  .and. associated(RMELTOC002)) RMELTOC002_enavg = RMELTOC002_enavg + RMELTOC002

    ! Accumulate ensemble members (multi-dimensional fields)

    if (associated(RHOSNOW_enavg)   .and. associated(RHOSNOW))    RHOSNOW_enavg  = RHOSNOW_enavg   + RHOSNOW
    if (associated(TSNOW_enavg)     .and. associated(TSNOW))        TSNOW_enavg  = TSNOW_enavg     + TSNOW
    if (associated(TICE0_enavg)     .and. associated(TICE0))        TICE0_enavg  = TICE0_enavg     + TICE0
    if (associated(WSNOW_enavg)     .and. associated(WSNOW))        WSNOW_enavg  = WSNOW_enavg     + WSNOW
    if (associated(ZSNOW_enavg)     .and. associated(ZSNOW))        ZSNOW_enavg  = ZSNOW_enavg     + ZSNOW
    if (associated(DRHOS0_enavg)    .and. associated(DRHOS0))      DRHOS0_enavg  = DRHOS0_enavg    + DRHOS0
    if (associated(WESNEX_enavg)    .and. associated(WESNEX))      WESNEX_enavg  = WESNEX_enavg    + WESNEX
    if (associated(WESNPERC_enavg)  .and. associated(WESNPERC))  WESNPERC_enavg  = WESNPERC_enavg  + WESNPERC
    if (associated(WESNDENS_enavg)  .and. associated(WESNDENS))  WESNDENS_enavg  = WESNDENS_enavg  + WESNDENS
    if (associated(WESNREPAR_enavg) .and. associated(WESNREPAR)) WESNREPAR_enavg = WESNREPAR_enavg + WESNREPAR
